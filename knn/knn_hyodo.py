...
class NearestNeighbors(object):
    ...
    def predict(self, x):
        ...
        for i in range(len(self._train_data)):
            distances = np.append(distances, distance(x,self._train_data[i]))
...
class KNearestNeighbors(object):
    ...
    def predict(self, x):
        ...
        distances = []
        for i in range(len(self._train_data)):
            distances.append(distance(x,self._train_data[i]))

        # ｋ個の距離が近いラベルを求める
        targets = []
        for k in range(self._k):
            targets.append(self._target_data[np.argsort(distances)[k]])

        # 同じラベルの数を数える
        targets_u = np.unique(targets)
        counts = []
        for i in range(len(targets_u)):
            counts = np.append(counts, targets.count(targets_u[i]))

        # 一番多く表れたラベルを返す
        return targets_u[counts.argmax()]
